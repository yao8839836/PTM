...
(Note: Each line in above files is for a prescription, the same line in `/data/pre_herbs_X.txt` and `/data/pre_symptoms_X.txt` (X is train or test or '') is for the same prescription. Each number in above files means an herb or a symptom, each number is an index of the following herb list or symptom list. For example, '5' in /file/pre_herbs_train.txt means the 6th herb in the herb list /data/herbs_contains.txt, '17' in /file/pre_symptoms_train.txt means the 18th symptom in the symptom list /data/symptom_contains.txt. )
