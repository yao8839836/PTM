...
public class PTMd {
	...
	Map<Integer, Set<Integer>> herb_symptom;

	int iterations;

	public PTMd(int[][] herbs, int[][] symptoms, int H, int S, Map<Integer, Set<Integer>> herb_symptom) {
		...
		this.herb_symptom = herb_symptom;
	...
	public void gibbs() {
		...
		for (int p = 0; p < herbs.length; p++) {
			...
			Map<Integer, Set<Integer>> syndrome_assignment = new HashMap<>();
			...
			for (int n = 0; n < herb_pairs[p].length; n++) {
				...
				if (herb_symptom.containsKey(herb_pairs[p][n][0])) {

					Set<Integer> corr_symptoms = herb_symptom.get(herb_pairs[p][n][0]);

					for (int s : corr_symptoms) {

						if (!syndrome_assignment.containsKey(s)) {

							Set<Integer> topics = new HashSet<>();

							topics.add(treatment[p][n]);

							syndrome_assignment.put(s, topics);

						} else {

							Set<Integer> topics = syndrome_assignment.get(s);
				...
				if (herb_symptom.containsKey(herb_pairs[p][n][1])) {

					Set<Integer> corr_symptoms = herb_symptom.get(herb_pairs[p][n][1]);
			...
			for (int m = 0; m < symptoms[p].length; m++) {
				...
				if (!syndrome_assignment.containsKey(symptoms[p][m]))
					syn = sampleFullConditionalSyndrome(p, m);
				else {

					Set<Integer> topics = syndrome_assignment.get(symptoms[p][m]);

					// List<Integer> topic_list = new ArrayList<>(topics);

					syn = sampleFullConditionalSyndrome(p, m, topics);
				}
				syndrome[p][m] = syn;
	...
	int sampleFullConditionalSyndrome(int p, int m, List<Integer> topic_list) {

		int syn = syndrome[p][m];

		updateCountSymptom(p, syn, symptoms[p][m], -1);

		double[] pr = new double[topic_list.size()];

		for (int k = 0; k < pr.length; k++) {

			int syndrome = topic_list.get(k);

			pr[k] = (np[p][syndrome] + alpha) / (npsum[p] + K * alpha) * (ns[symptoms[p][m]][syndrome] + beta_bar)
					/ (nssum[syndrome] + S * beta_bar);
		}

		int index = sample(pr);

		syn = topic_list.get(index);
	...
	int sampleFullConditionalSyndrome(int p, int m, Set<Integer> topic_set) {
